...
@section('content')
	<div class="row">
		<div class="col-12">
			<form action="{{ route('admin.categories.'.request()->segment(3).'.children.store', $parent) }}" method="post" enctype="multipart/form-data">
				@csrf
							...
							<div class="card-body">
								...
								<div class="row">
									<div class="col-md-6 mb-3">
										...
										<input type="text" class="form-control" id="name" name="name" placeholder="@lang('admin/category.placeholders.name')" value="{{ old('name') }}" data-seo-title data-seo-link/>
									</div>

									<div class="col-md-6 mb-3">
										<label for="priority" class="form-label">@lang('admin/global.fields.priority')</label>
										<input type="number" class="form-control" id="priority" name="priority" placeholder="@lang('admin/global.placeholders.priority')" min="1" max="9999" value="{{ old('priority') }}"/>
									</div>
								</div>
